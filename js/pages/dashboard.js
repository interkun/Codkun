 ...
 * COMPLETE FIX: Android Path Cleaner, Mobile-Friendly UI, Firebase Save
 */
    ...
    if(importBtn && folderUploader) {
        ...
        folderUploader.onchange = async (e) => {
            ...
            for(let file of files) {
                ...
                // --- ANDROID PATH CLEANER FIX ---
                if(relativePath.includes('primary:')) {
                    try {
                        let decoded = decodeURIComponent(relativePath);
                        let parts = decoded.split('/');
                        // Remove Android system junk folders
                        let validParts = parts.filter(p => p !== 'tree' && p !== 'document' && !p.includes('primary:'));
                        relativePath = validParts.join('/') || file.name;
                    } catch(e) {
    ...
    function createTreeNode(nodeObj, container) {
        ...
        keys.forEach(key => {
            ...
            if (node.type === 'folder') {
                ...
                // MOBILE UI FIX: Removed 'hidden' from the button container so it's always visible
                header.innerHTML = `
                ...
                // MOBILE UI FIX: Removed 'hidden' so Edit and Delete are always tappable
                div.innerHTML = `
